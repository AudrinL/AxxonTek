...
/**
 * The motion layer, shared with the studio's other work (Inzovu, RwandAir).
 *
 * Lenis owns the wheel and GSAP's ScrollTrigger stays in step with it, so
 * every pinned stage, scrub and reveal on the site measures against the
 * same virtual scroll position. The rule everywhere: the resting state is
 * correct without JavaScript, motion only sharpens it, and everything is
...
/**
 * Scrubbed parallax: translate an element on the Y axis as its container
 * passes through the viewport. `depth` is the total travel in px; negative
 * moves against the scroll (rises), positive moves with it (sinks).
 */
export function useParallax<T extends HTMLElement>(depth = 80) {
  const ref = useRef<T | null>(null);
